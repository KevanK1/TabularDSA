...
app.post("/get-timetable", async (req, res) => {
    try {
        ...
        const timetableData = {
            teachers: teachers.map((t) => ({ id: t._id.toString(), name: t.name, email: t.email })),
            ...
            rooms: rooms.map((r) => ({ id: r._id.toString(), name: r.name, capacity: r.capacity })),
        ...
        const response = await axios.post("http://127.0.0.1:8000/generate-timetable", timetableData);
...
app.post("/generate-timetable", (req, res) => {
    res.status(404).send("Route not found. Did you mean to call the FastAPI endpoint at http://127.0.0.1:8000/generate-timetable?");
});
